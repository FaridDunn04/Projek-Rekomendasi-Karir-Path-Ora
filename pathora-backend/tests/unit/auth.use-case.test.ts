 ...
 *
 * Unit test untuk use-case auth (FR-001, FR-002, SEC-001, SDD §8.2, §8.3).
...
import { createLoginUseCase } from "@/services/auth/use-cases/login.use-case";
...
describe("login.use-case", () => {
  ...
  it("melempar AuthenticationError dengan pesan generik bila email tidak ditemukan (FR-002)", async () => {
    ...
    // Pesan harus generik — tidak menyebut "email" atau "password" secara spesifik
    await expect(
      useCase.execute({ email: "notfound@example.com", password: "secret" }),
