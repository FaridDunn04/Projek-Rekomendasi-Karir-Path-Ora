/**
 * tests/integration/analyses.integration.test.ts
 *
 * Integration test endpoint Analyses (API-010..API-013, SDD §8.2).
 * AI gateway menggunakan mock adapter (USE_MOCK_AI=true).
 */
...
describe("GET /api/v1/analyses/:analysisId", () => {
  it("200 — detail analisis dengan filtering tampilan (FR-016, FR-020)", async () => {
    ...
    // Response structure: { id, status, result: { top_5_predictions, ... } }
    const analysisData = res.body.data;
    expect(analysisData.id).toBeDefined();
    expect(analysisData.status).toBe("success");

    // Aturan filter: confidence > 0.05 (FR-016) — ada di dalam .result
    ...
    // Aturan filter: match_score > 0.3 (FR-020) — ada di dalam .result
